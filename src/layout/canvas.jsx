...
class Canvas extends React.Component {
            ...
            antialias: true
                        ...
                        this.state.points.forEach((point) => {
                            ...
                            el.addEventListener('click', () => {
                                ...
                                link.href = point.file;
                                link.download = point.id + '.pdf';
                                link.click();
                            });
	...
	this.setMapStyleListener = emitter.addListener('setMapStyle', e => {
		...
		Object.keys(currentSources).forEach(sourceId => {
		    if (!this.state.map.getSource(sourceId)) {
		        ...
		        // Source already exists in new style, skip
		    }
		});
		...
		currentLayers.forEach(layer => {
		    if (!this.state.map.getLayer(layer.id)) {
		        ...
		        // Layer already exists in new style, skip
		    }
		});
	    });
    ...
    componentWillUnmount() {
        emitter.removeAllListeners('setMapStyle');
        emitter.removeAllListeners('displayDataset');
        emitter.removeAllListeners('removeDataset');
        emitter.removeAllListeners('displayTempLayer');
        emitter.removeAllListeners('toggleLayerVisibility');
        emitter.removeAllListeners('changeLayerTransparency');
        emitter.removeListener('moveURL', this.handleURLMoved);
    }
