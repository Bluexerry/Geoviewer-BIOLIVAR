...
export default function HorizontalLinearStepperData({ onSubmit }) {
  ...
  const fetchEquipmentDataTypes = async (type) => {
    try {
      const response = await fetch(`${SENSOR_API_URL}/dataTypes/${type}`);
      const data = await response.json();
      setDataTypes(data);
    } catch (error) {
      // Failed to fetch equipment data types — handled silently
    }
  };
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`${SENSOR_API_URL}/data`, {
        method: 'POST',
      ...
      // Data fetch failed — no action needed
    }
  };
